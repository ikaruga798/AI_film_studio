# ⚠️ 声明
本项目代码完全开源，不内置任何付费解锁功能，不接入任何第三方广告或代理API。如果您从任何渠道被要求付费使用本程序，请立即举报——收费的一定是骗子！
...
移除了画幅比选项；
集成火山引擎与谷歌AI Studio图片/视频生成预设，支持全局预设保存与导入；
优化了请求处理逻辑（字段留空不发送，无图片参考时不发送图片字段）

本程序与常见ai电影创作流程区别类似洗衣机与手洗衣服，能够提高生成效率，熟悉后比起这次==正常操作平均用时能够直接减半，但工作流无法提升视频质量，该抽卡还得抽卡

# 风险提示
使用本程序调用第三方AI API时，您需要自行承担API调用产生的费用（如有）以及数据安全责任。本程序仅作为工具辅助创作，不提供任何形式的担保。

# 开源协议
本项目基于MIT协议开源，您可以自由使用、修改、分发，但需保留原作者版权声明。
...
下载并打开 VSCode，安装python，新建一个venv虚拟环境，确认你已经在 venv 虚拟环境里（终端左边应该有 `(venv)` 字样）。
